...
@_register("base64_decode", "decode", "Base64 decode", ["input"])
...
@_register("unicode_encode", "encode", "Unicode escape encode (\\uXXXX)", ["input"])
def _unicode_encode(input_str: str, **_) -> dict:
    encoded = input_str.encode("unicode_escape").decode("ascii")
    return {"success": True, "result": encoded}


@_register("unicode_decode", "decode", "Unicode escape decode (\\uXXXX)", ["input"])
...
@_register("rot13_encode", "encode", "ROT13 encode (self-inverse; encode == decode)", ["input"])
...
@_register(
    "caesar_encode", "encode", "Caesar cipher encode (shift cipher)", ["input"], {"shift": "shift amount, default 3"}
)
...
@_register(
    "caesar_decode",
    "decode",
    "Caesar cipher decode (brute-force all shifts)",
    ["input"],
    {"shift": "shift amount; if omitted, returns all 25 possibilities"},
)
...
@_register("morse_encode", "encode", "Morse code encode", ["input"])
...
@_register("morse_decode", "decode", "Morse code decode", ["input"])
...
@_register("jwt_decode", "decode", "JWT decode (Header + Payload)", ["input"])
    ...
    try:
        ...
        if len(parts) != 3:
            return {
                "success": False,
                "result": "",
                "error": "JWT must contain 3 parts (header.payload.signature)",
            }
...
@_register(
    "jwt_encode",
    "encode",
    "JWT encode (requires header, payload, secret)",
    ["input"],
    {"header": "JWT header JSON", "secret": "signing key", "algorithm": "signing algorithm, default HS256"},
)
def _jwt_encode(
    input_str: str,
...
@_register(
    "aes_encrypt",
    "encrypt",
    "AES encrypt (CBC mode, PKCS7 padding)",
    ["input"],
    {"key": "key (16/24/32 bytes)", "iv": "initialization vector (16 bytes, defaults to the key)"},
)
...
@_register(
    "aes_decrypt",
    "decrypt",
    "AES decrypt (CBC mode, PKCS7 padding)",
    ["input"],
    {"key": "key (16/24/32 bytes)", "iv": "initialization vector (16 bytes, defaults to the key)"},
)
...
@_register("auto_decode", "decode", "Auto-detect the encoding type and decode (tries all common encodings)", ["input"])
def _auto_decode(input_str: str, **_) -> dict:
    ...
    if "&" in s and ("#" in s or s.count(";") > 0):
        try:
            ...
            if decoded != s:
                results.append(f"[HTML decode] {decoded}")
        except Exception:
            pass
    ...
    if "\\u" in s:
        try:
            ...
            results.append(f"[Unicode decode] {decoded}")
        except Exception:
            pass
    ...
    if re.match(r"^[A-Za-z0-9_-]+$", s) and len(s) >= 4:
        try:
            ...
            if decoded and any(c.isprintable() for c in decoded):
                results.append(f"[Base64URL decode] {decoded}")
        except Exception:
            pass
    ...
    if re.match(r"^[A-Z2-7]+=*$", s.upper()) and len(s) >= 8:
        try:
            ...
            if decoded:
                results.append(f"[Base32 decode] {decoded}")
        except Exception:
            pass
    ...
    if set(s) <= {".", "-", " ", "/"} and ("." in s or "-" in s):
        try:
            ...
            if decoded["success"]:
                results.append(f"[Morse decode] {decoded['result']}")
        except Exception:
            pass
    ...
    if s.isalpha():
        ...
        try:
            ...
            if decoded != s:
                results.append(f"[ROT13 decode] {decoded}")
        except Exception:
            pass

    if not results:
        return {"success": False, "result": "", "error": "Could not auto-detect the encoding type"}
...
def execute(operation: str, input_str: str, **kwargs) -> dict:
    ...
    if operation not in OPERATIONS:
        ...
        return {
            "success": False,
            "result": "",
            "error": f"Unknown operation: {operation}. Available operations: {available}",
        }
    ...
    try:
        ...
        return {"success": False, "result": "", "error": f"Error executing {operation}: {e}"}
